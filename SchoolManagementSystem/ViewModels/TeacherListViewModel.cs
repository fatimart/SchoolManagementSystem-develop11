...
namespace SchoolManagementSystem.ViewModels
{
    class TeacherListViewModel : ViewModelBase
    {
        ...
        public void InsertUser ( string username, string name, string email, decimal cpr, string address, DateTime dob, string password, string contactNo )
        {
            try
            {
                ...
                if (user1.UserID <= 0)
                {
                    ty.Users.Add(user1);
                    ty.SaveChanges();
                    MessageBox.Show("New teacher successfully saved.");
        ...
        {
            if (MessageBox.Show("Confirm delete of this record?", "Teacher", MessageBoxButton.YesNo)
        ...
        public bool CheckIfUserExists ( int userID )
        {
            try
            {
                var user1 = ty.Users.Where(m => m.UserID == userID).Single();
                return true;
            }
            catch (Exception ex)
            {
